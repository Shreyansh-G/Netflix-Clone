	...
	return (
		<div className='hero-bg min-h-screen relative'>
			...
			<header className='max-w-6xl mx-auto flex items-center justify-between p-4 pb-10'>
				...
				<Link to={"/login"} className='text-white bg-red-600 py-2 px-4 text-lg rounded'>
					Sign In
				</Link>
			</header>
			...
			<div className='flex flex-col items-center justify-center text-center py-40 text-white max-w-6xl mx-auto'>
				...
				<form className='flex flex-col md:flex-row gap-4 w-full md:w-2/3' onSubmit={handleFormSubmit}>
					<input
						type='email'
						placeholder='Email address'
						className='p-4 text-lg rounded flex-1 bg-black/80 border border-gray-700'
						value={email}
						onChange={(e) => setEmail(e.target.value)}
					/>
					<button
						className='bg-red-600 text-lg lg:text-2xl px-6 py-4 rounded flex justify-center items-center'
					>
						Get Started
						<ChevronRight className='ml-2 w-6 h-6 lg:w-8 lg:h-8' />
					</button>
				</form>
			</div>
